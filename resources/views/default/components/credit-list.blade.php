...
@else
	@php
		...
		$uniqueDriversByDefaultImageModel = $imageEntities?->list()
	...
	>
		@if ($showType !== 'button')
			<div
				class="{{ @twMerge($style === 'inline' ? 'lqd-remaining-credits-legends flex items-center justify-between gap-3 flex-wrap' : '', $attributes->get('class:legends')) }}">
				@if ($aiImage)
				...
				@else
					...
					>
                        <span class="ms-auto font-medium">
                            {{ $wordContainUnlimited ? __('Unlimited') : $wordCreditsCount }}
                        </span>
					</x-legend>
				@endif
				...
				>
                    <span class="ms-auto font-medium">
                        {{ $imageContainUnlimited ? __('Unlimited') : $imageCreditsCount }}
                    </span>
				</x-legend>
			</div>
...
<script>
	@if($isJs)
		document.addEventListener('DOMContentLoaded', function () {
			fetch('{!! route('dashboard.user.payment.credit-list-partial', ['cache_key' => request('credit-list-cache'), 'plan_id' => $plan?->id]) !!}')
