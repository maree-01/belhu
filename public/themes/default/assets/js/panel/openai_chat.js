...
function onConversationAreaScroll() {
	...
	conversationAreaScrollLocked =
		...
		(conversationArea.scrollTop + conversationArea.offsetHeight + 10) >= conversationArea.scrollHeight;
}
	...
	const wrapper = (function() {
		...
		function wrapWordsInner(node, options = {}) {
			if (node.nodeName === 'PRE' || node.nodeName === 'CODE') {
				...
				if (options.style) {
					options.style.split(';').forEach(style => {
...
* @param {object} options.startingKeyframes
*/
function formatString( string, options = {} ) {
	...
	if ( options.readyForAnimation ) {
		...
		if ( options.startingKeyframes ) {
			Object.entries(options.startingKeyframes).forEach(([ key, value ]) => {
				style += `${key}: ${value};`;
			});
		}

		wrapWords(html, { className: 'animated-word inline-block', style });
...
const handleConversationsAreaScroll = _.throttle( word => {
	...
	animatingWordOffsetY = aiBubbleOffsetTop - (aiBubbleHeight / 2) + (conversationArea.offsetHeight / 2) + wordOffsetTop + 20;

	conversationAreaScrollLocked && scrollConversationArea({ y: animatingWordOffsetY, smooth: true });
}, 150, { leading: true } );
...
function onAiResponse() {
	...
	// calling clean is causing code blocks formatting meesed up
	// vdomDiffInstance.clean(dom);
