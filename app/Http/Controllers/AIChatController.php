...
class AIChatController extends Controller
{
    ...
    public function __construct(BedrockRuntimeService $bedrockService)
    {
        ...
        $this->getOpenAiApiKey(Auth::user());
    }
    ...
    private function firstOpenaiGeneratorChatCategory(?string $slug = null)
    {
        ...
        $generator = OpenaiGeneratorChatCategory::query()
            ->whereNotIn('slug', [
    ...
    {
        $apiKeys = $this->settings->user_api_option
            ? explode(',', $user?->api_keys)
            : explode(',', $this->settings->openai_api_secret);
